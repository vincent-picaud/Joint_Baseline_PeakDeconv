...
main(int argc, char* argv[])
{
  ...
  options.add_options()
      //
      ("i,input", "Input file (two columns X,Y)", cxxopts::value<std::string>(), "FILE")
      //
      ("o,output", "Output file", cxxopts::value<std::string>()->default_value("$(FILE).out"), "OUTPUT FILE")
      //
      ("sigma_left", "Peak shape factor (>0)", cxxopts::value<double>(sigma_left), to_string(sigma_left))
      //
      ("sigma_right", "Peak shape factor (>0)", cxxopts::value<double>(sigma_right), to_string(sigma_right))
      //
      ("yb_left", "Left baseline value (if not defined use y[0])", cxxopts::value<double>(yb_left), "y[0]")
      //
      ("yb_right", "Right baseline value (if not defined use y[n-1])", cxxopts::value<double>(yb_right), "y[n-1]")
      //
      ("peakMinHeight",
      ...
      //
      ("lambda_1", "lambda_1 penalty term (>=0)", cxxopts::value<double>(lambda_1), to_string(lambda_1))
      //
      ("lambda_2", "lambda_2 penalty term (>=0)", cxxopts::value<double>(lambda_2), to_string(lambda_2))
      //
      ("mu", "mu penalty term (>0)", cxxopts::value<double>(mu), to_string(mu))
      //
      ("eps", "eps goal (>=0)", cxxopts::value<double>(eps), to_string(eps))
      //
      ("max_iter", "maximum number of iterations (>0)", cxxopts::value<Size_t>(max_iter), to_string(max_iter))
      //
  ...
  if (sigma_left <= 0)
  {
    std::cout << "#Error: sigma_left= " << sigma_left << " is not a positive number" << std::endl;
    exit(-1);
  }
  if (sigma_right <= 0)
  {
    std::cout << "#Error: sigma_right= " << sigma_right << " is not a positive number" << std::endl;
    exit(-1);
  }
  if (peakMinHeight < 0)
  {
    std::cout << "#Error: peakMinHeight= " << peakMinHeight << " is not a nonegative number" << std::endl;
    exit(-1);
  }
  if (lambda_1 < 0)
  {
    std::cout << "#Error: lambda_1= " << lambda_1 << " is not a nonnegative number" << std::endl;
    exit(-1);
  }
  if (lambda_2 < 0)
  {
    std::cout << "#Error: lambda_2= " << lambda_2 << " is not a nonnegative number" << std::endl;
    exit(-1);
  }
  if (mu <= 0)
  {
    std::cout << "#Error: mu= " << mu << " is not a positive number" << std::endl;
    exit(-1);
  }
  if (eps < 0)
  {
    std::cout << "#Error: eps= " << eps << " is not a nonnegative number" << std::endl;
    exit(-1);
  }
  if (max_iter <= 0)
  {
    std::cout << "#Error: max_iter= " << max_iter << " is not a positive number" << std::endl;
    exit(-1);
  }

  // Generates output filename
  //
  const std::string output_filename((options.count("output") == 0) ? options["input"].as<std::string>() + ".out"
  ...
  for (Index_t i = 0; i < n; ++i)
  {
    output << x[i] << ", " << y[i] << ", " << deconvolvedPeak[i] << ", " << baseline[i] << ", " << convolvedPeak[i]
  ...
  if (gnuplot)
  {
    ...
    output_gnuplot << "plot \"" << output_filename << "\" u 1 : 2 w l t \"Y raw\" lc \"blue\"" << std::endl;
    output_gnuplot << "replot \"" << output_filename << "\" u 1 : 6 w l t \"reconstructed\" lw 2 lc \"grey30\""
                   << std::endl;
    output_gnuplot << "replot \"" << output_filename << "\" u 1 : 4 w l t \"baseline\" lw 2 lc \"grey70\"" << std::endl;
    output_gnuplot << "replot \"" << output_filename << "\" u 1 : 5 w l t \"convolved peaks\" lc \"green\""
                   << std::endl;
    output_gnuplot << "replot \"" << output_filename << "\" u 1 : 3 w i t \"peaks\" lw 2 lc \"red\"" << std::endl;
